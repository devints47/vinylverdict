...
export async function POST(request: NextRequest) {
  try {
    ...
    // Calculate expiry time (subtract 2 minutes for safety margin)
    const expiryTime = Date.now() + (tokenData.expires_in - 120) * 1000

    // Set cookies
    cookieStore.set("spotify_access_token", tokenData.access_token, {
      ...cookieOptions,
      maxAge: tokenData.expires_in - 120, // Expires_in minus 2 minutes in seconds
    })
    ...
    cookieStore.set("spotify_token_expiry", expiryTime.toString(), {
      ...cookieOptions,
      maxAge: tokenData.expires_in - 120,
    })
