...
function parseRoastText(text: string): Array<{ type: string; content: string; style?: any; isMarkdown?: boolean }> {
  ...
  if (paragraphCount > 10) {
    ...
    baseFontSize = Math.max(baseFontSize - 2, 18)
  }

  const titleFontSize = Math.max(baseFontSize + 10, 36)
  const scoreFontSize = Math.max(baseFontSize + 6, 32)
