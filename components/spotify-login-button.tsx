...
export function SpotifyLoginButton({ className, text = "Log in", showHoverEffect = false }: SpotifyLoginButtonProps) {
  ...
  return (
    <Button
      ...
      className={`relative min-w-[226px] overflow-hidden btn-gradient holographic-shimmer text-white font-bold py-3 px-8 rounded-full text-lg transition-all duration-300 ${showHoverEffect ? "hover:scale-110 hover:shadow-purple-glow" : "hover:scale-105"} flex items-center gap-2 shadow-lg ${className || ""}`}
    >
